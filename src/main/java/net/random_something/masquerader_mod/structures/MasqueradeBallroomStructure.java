...
public class MasqueradeBallroomStructure extends Structure {
    ...
    public MasqueradeBallroomStructure(StructureSettings p_227593_) {
        super(p_227593_);
    }
    ...
    public Optional<GenerationStub> findGenerationPoint(GenerationContext context) {
        ...
        return Optional.of(new GenerationStub(blockpos, (p_228526_) -> generatePieces(p_228526_, context)));
    }

    private boolean isSurfaceExposedToSky(GenerationContext context, ChunkGenerator chunkGenerator, BlockPos pos) {
    ...
    private int getTerrainHeightAtPosition(GenerationContext context, ChunkGenerator chunkGenerator, BlockPos pos) {
    ...
    private boolean isSlopeTooSteep(GenerationContext context, ChunkGenerator chunkGenerator, BlockPos pos) {
    ...
    private boolean isNearWater(GenerationContext context, ChunkGenerator chunkGenerator, BlockPos pos) {
        int radius = 5;
    ...
    private BlockPos findDryLandNearby(GenerationContext context, ChunkGenerator chunkGenerator, BlockPos pos) {
        int searchRadius = 10;
            ...
            for (int l = boundingBox.minZ(); l <= boundingBox.maxZ(); ++l) {
                ...
                if (!level.isEmptyBlock(blockpos$mutableblockpos) && boundingbox.isInside(blockpos$mutableblockpos) && p_230233_.isInsidePiece(blockpos$mutableblockpos)) {
                    for (int i1 = j - 1; i1 > i; --i1) {
                        ...
                        if (!level.isEmptyBlock(blockpos$mutableblockpos) && !level.getBlockState(blockpos$mutableblockpos).getMaterial().isLiquid()) {
                            break;
                        }
